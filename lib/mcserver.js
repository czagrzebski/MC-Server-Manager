...
class MCServer extends EventEmitter {
    ...
    startServer = async () => {
        if(this.state === STATES.STOPPED) {
            this.serverProcess = spawn("java", this.launchOptions, {cwd: './servers/test-server'});
    ...
    readProperties = async () => {
        ...
        const propertiesFile = await fs.readFile(path.join(__dirname, '../servers/test-server/server.properties'), 'utf-8')
            .catch((err) => {
                if (err.code === "ENOENT") throw new Error("Server Properties File does not exist. Start the server and try again");
            })
