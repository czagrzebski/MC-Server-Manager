...
class MCServer extends EventEmitter {
    ...
    startServer = async () => {
        ...
        this.serverProcess = spawn("java", launchOptions, {
            cwd: (path.join(__dirname, `..${config.folderDir}/`))
        });

        console.log((path.join(__dirname, `..${config.folderDir}`)))
     ...
     */
      isEulaSigned = async () => {
        ...
        return fs.readFile(path.join(__dirname, `..${config.folderDir}/eula.txt`), 'utf-8')
            .then(propertiesFile => {
    ...
    getServerConfig = async () => {
        ...
        return JSON.parse(userConfig)["servers"][this.UUID]
    }
    ...
    /**
     * Fetches the user and server.properties configuration.
    ...
    buildServerOptions = () => {
        ...
        return this.getServerConfig()
            .then(config => {
                if (config.java.maxHeapSize >= 12000) { //if memory is greater than 12GB, use alternative experimental options
                ...
                return `-Xmx${config.java.maxHeapSize}M -Xms${config.java.maxHeapSize}M ${experimentalFlags} -jar ${config.java.jarFile} nogui`.split(' ')
            })
