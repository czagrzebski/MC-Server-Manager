...
class MCServer extends EventEmitter {
     ...
     * 
     */
    setServerSettings = async (category, setting, value) => {
        switch (category) {
            case "java":
            case "core":
    ...
    buildServerOptions = () => {
        ...
        return this.getServerConfig()
            .then(config => {
                if (config.maxHeapSize.value >= 12000) { //if memory is greater than 12GB, use alternative experimental options
                ...
                return `-Xmx${config.maxHeapSize.value}M -Xms${config.maxHeapSize.value}M ${experimentalFlags} -jar ${config.jarFile.value} nogui`.split(' ')
            })
